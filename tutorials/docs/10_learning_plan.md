...
这一阶段已经完成第一轮主链路学习。你已经通过 debug 走通了同步 `chat` 接口，并理解了：

1. `session_id -> conversation` 的会话管理逻辑
2. `Conversation` 与 `Message` 的分层存储
3. 用户画像与学习进度作为业务上下文的收集
4. orchestrator 的意图识别与 agent 路由
5. QA Agent 进入 RAG 链路，其他 Agent 更偏 Prompt 驱动生成
6. AI 结果返回后再次落库，并触发异步画像/进度更新
...
后续你将把这一阶段作为主攻方向之一，尤其重点关注：

1. RAGGenerator 与 Retriever 的数据流转
2. hybrid retrieve 的权重设计
3. Milvus 在项目中的真实使用方式
4. 是否适合在现有 RAG 基础上补充联网搜索能力
...
Memory 已经成为你的后续学习主线之一。下一轮学习时建议重点观察：

1. 当前记忆系统是“已实现并深度接入”，还是“基础设施已具备但接入有限”
2. 短期、工作、长期记忆的数据边界是否清晰
3. 是否存在适合你补强的接入点
...
2. 接下来优先深挖 `chat -> orchestrator -> QA Agent -> RAG`
3. 然后单独抽一轮学习 memory
4. 再重点补一轮 Milvus 与检索链路
5. 再回到 profile、progress、async tasks 看 AI 如何落地成业务
6. 最后开始找优化点和可迭代功能
